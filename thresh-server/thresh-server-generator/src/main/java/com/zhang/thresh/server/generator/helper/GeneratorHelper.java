package com.zhang.thresh.server.generator.helper;

import com.zhang.thresh.common.core.annotation.Helper;
...
import com.zhang.thresh.server.generator.entity.FieldType;
...
public class GeneratorHelper {
    ...
    public void generateEntityFile(List<Column> columns, GeneratorConfig configure) throws Exception {
        ...
        columns.forEach(c -> {
            c.setField(ThreshUtil.underscoreToCamel(StringUtils.lowerCase(c.getName())));
    ...
    public void generateMapperXmlFile(List<Column> columns, GeneratorConfig configure) throws Exception {
        ...
        JSONObject data = toJsonObject(configure);
        columns.forEach(c -> c.setField(ThreshUtil.underscoreToCamel(StringUtils.lowerCase(c.getName()))));
        data.put("columns", columns);
    ...
    private Template getTemplate(String templateName) throws Exception {
        ...
        if (!file.exists()) {
            templatePath = System.getProperties().getProperty(ThreshConstant.JAVA_TEMP_DIR);
        ...
        configuration.setDefaultEncoding(ThreshConstant.UTF8);
