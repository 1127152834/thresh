...
import com.zhang.thresh.common.core.entity.CurrentUser;
...
import com.zhang.thresh.common.core.entity.system.UserRole;
...
import com.zhang.thresh.server.system.mapper.UserMapper;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, SystemUser> implements IUserService {
    ...
    public IPage<SystemUser> findUserDetailList(SystemUser user, QueryRequest request) {
        ...
        SortUtil.handlePageSort(request, page, "userId", ThreshConstant.ORDER_ASC, false);
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public void updateProfile(SystemUser user) throws ThreshException {
        user.setPassword(null);
        user.setUsername(null);
        user.setStatus(null);
        if (isCurrentUser(user.getUserId())) {
            ...
            throw new ThreshException("您无权修改别人的账号信息！");
        }
    }
    ...
    public void updateSidebarTheme(String theme) {
        CurrentUser currentUser = ThreshUtil.getCurrentUser();
        if (currentUser != null) {
