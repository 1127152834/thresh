...
import com.zhang.thresh.gateway.enhance.mapper.RateLimitLogMapper;
    ...
    private Query getQuery(RateLimitLog rateLimitLog) {
        ...
        if (StringUtils.isNotBlank(rateLimitLog.getCreateTimeFrom())
            ...
            criteria.andOperator(
                    Criteria.where(ThreshConstant.CREATE_TIME).gt(rateLimitLog.getCreateTimeFrom()),
                    Criteria.where(ThreshConstant.CREATE_TIME).lt(rateLimitLog.getCreateTimeTo())
            );
        }
