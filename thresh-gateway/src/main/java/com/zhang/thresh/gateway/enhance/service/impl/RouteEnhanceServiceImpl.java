...
import com.zhang.thresh.gateway.enhance.entity.*;
import com.zhang.thresh.gateway.enhance.service.*;
...
public class RouteEnhanceServiceImpl implements RouteEnhanceService {
    ...
    public Mono<Void> filterBlackList(ServerWebExchange exchange) {
        ...
        try {
            ...
            if (originUri != null) {
                String requestIp = ThreshUtil.getServerHttpRequestIpAddress(request);
                ...
                if (forbid.get()) {
                    return ThreshUtil.makeWebFluxResponse(response, MediaType.APPLICATION_JSON_VALUE,
                            HttpStatus.NOT_ACCEPTABLE, new ThreshResponse().message("黑名单限制，禁止访问"));
                }
            } else {
    ...
    public void saveRequestLogs(ServerWebExchange exchange) {
        ...
        if (!StringUtils.equalsIgnoreCase(TOKEN_CHECK_URL, originUri.getPath())) {
            ...
            ServerHttpRequest request = exchange.getRequest();
            String ipAddress = ThreshUtil.getServerHttpRequestIpAddress(request);
    ...
    private Mono<Void> doRateLimitCheck(AtomicBoolean limit, RateLimitRule rule, URI uri,
        ...
        if (limit.get()) {
            ...
            if (count == 0) {
                ...
                return ThreshUtil.makeWebFluxResponse(response, MediaType.APPLICATION_JSON_VALUE,
                        HttpStatus.TOO_MANY_REQUESTS, new ThreshResponse().message("访问频率超限，请稍后再试"));
            } else {
