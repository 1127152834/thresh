...
import com.zhang.thresh.gateway.enhance.mapper.BlockLogMapper;
    ...
    private Query getQuery(BlockLog blockLog) {
        ...
        if (StringUtils.isNotBlank(blockLog.getCreateTimeFrom())
            ...
            criteria.andOperator(
                    Criteria.where(ThreshConstant.CREATE_TIME).gt(blockLog.getCreateTimeFrom()),
                    Criteria.where(ThreshConstant.CREATE_TIME).lt(blockLog.getCreateTimeTo())
            );
        }
